...
// @downloadURL   https://raw.githubusercontent.com/kevingrillet/Userscripts/main/user.js/deprecated/[Amazon]%20Tweaks%20(Wishlist).user.js
...
var cleanUI = true, // remove crap ui
    loadSpeed = 0.1 * 1000, //Refresh speed during load.
    maxLoadingTry = 30;
...
function loadAll() {
    if (!document.querySelector('#endOfListMarker') && loadingTry < maxLoadingTry) {
        setTimeout(function () {
            [document.querySelectorAll(':scope #g-items li').length - 1].scrollIntoView(/*{behavior: "smooth"}*/);
            loadingTry++;
            loadAll();
        }, loadSpeed);
    } else {
